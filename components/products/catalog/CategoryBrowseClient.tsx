      ...
      <div className="mx-auto max-w-6xl space-y-10 md:space-y-12">
        ...
        <RevealOnScroll>
          <section aria-labelledby="category-list-heading">
            ...
            {filtered.length === 0 ? (
              <div className="mt-8 rounded-[1.75rem] border border-dashed border-teal-300/70 bg-white/60 px-8 py-14 text-center">
        ...
        <RevealOnScroll>
          <div
            className="relative overflow-hidden rounded-[2rem] border border-white/40 p-8 shadow-[0_16px_64px_-16px_rgba(15,23,42,0.14)] backdrop-blur-xl md:p-10"
            style={{
              background:
                "linear-gradient(140deg, rgba(153,246,228,0.45) 0%, rgba(167,139,250,0.4) 50%, rgba(196,181,253,0.4) 100%)",
            }}
          ...
          >
            ...
            <div className="relative">
              <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-700">
                Custom synthesis
              </p>
              <h2
                id="category-cta-heading"
                className="mt-2 font-display text-2xl font-bold text-slate-950 md:text-3xl"
              >
              ...
              <p className="mt-3 max-w-2xl text-sm leading-relaxed text-slate-700 md:text-base">
                ...
                <span className="font-medium text-slate-900">{category.name}</span> space — reference this
                family in your RFQ.
              </p>
              <ButtonLink
        ...
        <RevealOnScroll>
          <section
            className="relative overflow-hidden rounded-[2rem] border border-white/50 px-8 py-10 backdrop-blur-sm md:px-12"
            style={{
              background:
                "linear-gradient(145deg, rgba(255,255,255,0.9) 0%, rgba(249,250,251,0.85) 100%)",
            }}
            ...
            <ul className="mt-8 grid gap-6 md:grid-cols-3">
              {WHY_ITEMS.map((item, i) => (
                <RevealOnScroll key={item.title} delay={i * 70}>
                  <li
                    className={`group h-full overflow-hidden rounded-[1.75rem] border border-white/60 bg-gradient-to-b ${item.tint} p-7 shadow-[0_8px_32px_-8px_rgba(15,23,42,0.07)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_48px_-16px_rgba(15,23,42,0.12)]`}
                  >
                    <div
