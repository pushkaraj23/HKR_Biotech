  ...
  return (
    <section className={SECTION_SHELL}>
      <div className="flex shrink-0 items-center justify-between gap-3">
        ...
        <Link
          href="/contact"
          className="rounded-full border border-on-dark/35 bg-[rgba(18,25,35,0.5)] px-4 py-2 text-xs font-semibold text-on-dark transition hover:border-primary/40"
        >
          New enquiry
        </Link>
      </div>
      ...
      <div className="mt-5 min-h-0 flex-1">
        {fetching ? (
          ...
          <ul className="max-h-[min(28rem,60vh)] space-y-2 overflow-y-auto pr-1 commerce-summary-scroll">
            {rows.map((row) => (
              <li key={row.id}>
                <article className="rounded-xl border border-on-dark/20 bg-[rgba(18,25,35,0.45)] p-3.5">
                  <div className="flex flex-wrap items-center gap-2">
                    <span
                      className={`rounded-full border px-2.5 py-0.5 text-[10px] font-semibold shadow-[0_6px_18px_-8px_rgba(0,0,0,0.45)] ${statusBadgeClass(row.status)}`}
                    >
                      {statusLabel(row.status)}
                    </span>
                    {row.reference ? (
                      <span className="font-mono text-[10px] text-on-dark/75">Ref: {row.reference}</span>
                    ) : null}
                    <span className="text-[10px] text-on-dark/65">{formatDate(row.createdAtIso)}</span>
                  </div>
                  <p className="mt-2 line-clamp-2 text-sm text-on-dark/86">{row.message}</p>
                  <p className="mt-1.5 text-[10px] text-on-dark/65">Source: {row.source || "website"}</p>
                </article>
              </li>
            ))}
          </ul>
